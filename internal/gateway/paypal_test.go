...
func TestPayPalCreatePaymentCreatesOrder(t *testing.T) {
	...
	gw, err := gateway.NewPayPalProvider(gateway.PayPalConfig{
		ClientID:     "client",
		ClientSecret: "secret",
		BaseURL:      srv.URL,
	})
	if err != nil {
		t.Fatalf("NewPayPalProvider: %v", err)
	}
	...
	if out.Provider != "paypal" || out.Method != string(gateway.CapabilityBrowserButton) {
...
func TestPayPalCapturePaymentCapturesApprovedOrder(t *testing.T) {
	...
	out, err := gw.CapturePayment(context.Background(), gateway.CapturePaymentIn{
		OrderNo:     "ORD-PP-1",
...
func TestPayPalRefundPaymentUsesCaptureRefund(t *testing.T) {
	...
	gw := newPayPalTestProvider(t, srv.URL)
	out, err := gw.Refund(context.Background(), gateway.RefundIn{
		OrderNo:      "ORD-PP-1",
...
func TestPayPalCaptureRejectsAmountMismatch(t *testing.T) {
	...
	gw := newPayPalTestProvider(t, srv.URL)
	_, err := gw.CapturePayment(context.Background(), gateway.CapturePaymentIn{
		OrderNo:     "ORD-PP-1",
		SessionID:   "PAYPAL-ORDER-1",
		AmountCents: 1234,
	})
...
func newPayPalTestProvider(t *testing.T, baseURL string) gateway.PaymentGateway {
	t.Helper()
	gw, err := gateway.NewPayPalProvider(gateway.PayPalConfig{
		ClientID:     "client",
		ClientSecret: "secret",

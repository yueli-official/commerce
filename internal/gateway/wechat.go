...
func NewWeChatProvider(cfg WeChatConfig) (PaymentGateway, error) {
...
func (p *wechatProvider) CreatePayment(ctx context.Context, in CreateIn) (*CreatePaymentOut, error) {
	if p.nativeSvc == nil {
	...
	req := native.PrepayRequest{
		...
		Description: wechatCore.String(truncatePayPalDescription(in.Subject)),
	...
	return &CreatePaymentOut{
		Provider:  "wechat",
		Method:    string(CapabilityNativeQR),
		SessionID: in.OrderNo,
		QRCode:    *resp.CodeUrl,
	}, nil
}

func (p *wechatProvider) CapturePayment(context.Context, CapturePaymentIn) (*CapturePaymentOut, error) {
	return nil, ErrUnsupportedOperation
}

func (p *wechatProvider) VerifyNotify(ctx context.Context, body []byte, headers map[string]string) (*NotifyOut, error) {
	if p.notifySvc == nil {
...
func (p *wechatProvider) Refund(ctx context.Context, in RefundIn) (*RefundOut, error) {
	if p.refundSvc == nil {
	...
	return &RefundOut{Success: success, ProviderID: *refund.RefundId, AmountCents: in.AmountCents}, nil
}

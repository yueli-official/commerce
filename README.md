...
`paykit` 是 Commerce 自有的支付 provider 工具包，负责 provider 中立的支付契约及具体 adapter：
...
只有 Commerce 可以直接使用 `paykit`。`products/shop/web` 等商店前端和 `products/shop/api` 等目录服务不得
导入；它们应调用 Commerce 的结算与订单接口。Commerce 负责订单快照、金额验证、履约、支付事件、退款和未来
清结算账本。
